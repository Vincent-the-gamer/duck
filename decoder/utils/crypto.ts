export async function sha256(data: Uint8Array): Promise<Uint8Array> {
  if (globalThis.crypto && globalThis.crypto.subtle && globalThis.isSecureContext) {
    try {
      const buffer = await globalThis.crypto.subtle.digest('SHA-256', data);
      return new Uint8Array(buffer);
    } catch (e) {
      // Fallback to JS implementation
    }
  }
  return sha256Js(data);
}

function sha256Js(message: Uint8Array): Uint8Array {
  const K = [
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
  ];

  function rotr(n: number, x: number): number {
    return (x >>> n) | (x << (32 - n));
  }

  function ch(x: number, y: number, z: number): number {
    return (x & y) ^ (~x & z);
  }

  function maj(x: number, y: number, z: number): number {
    return (x & y) ^ (x & z) ^ (y & z);
  }

  function sigma0(x: number): number {
    return rotr(2, x) ^ rotr(13, x) ^ rotr(22, x);
  }

  function sigma1(x: number): number {
    return rotr(6, x) ^ rotr(11, x) ^ rotr(25, x);
  }

  function gamma0(x: number): number {
    return rotr(7, x) ^ rotr(18, x) ^ (x >>> 3);
  }

  function gamma1(x: number): number {
    return rotr(17, x) ^ rotr(19, x) ^ (x >>> 10);
  }

  const len = message.length * 8;
  const extra = (message.length + 9) % 64;
  const padLen = extra === 0 ? 0 : 64 - extra;
  const newLen = message.length + 1 + padLen + 8;
  const buf = new Uint8Array(newLen);
  buf.set(message);
  buf[message.length] = 0x80;
  const high = Math.floor(len / 0x100000000);
  const low = len >>> 0;
  const view = new DataView(buf.buffer);
  view.setUint32(newLen - 8, high, false);
  view.setUint32(newLen - 4, low, false);

  let H = [
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
  ];

  const W = new Uint32Array(64);
  for (let i = 0; i < newLen; i += 64) {
    for (let t = 0; t < 16; t++) {
      W[t] = view.getUint32(i + t * 4, false);
    }
    for (let t = 16; t < 64; t++) {
      W[t] = (gamma1(W[t - 2]) + W[t - 7] + gamma0(W[t - 15]) + W[t - 16]) >>> 0;
    }
    let a = H[0];
    let b = H[1];
    let c = H[2];
    let d = H[3];
    let e = H[4];
    let f = H[5];
    let g = H[6];
    let h = H[7];
    for (let t = 0; t < 64; t++) {
      const T1 = (h + sigma1(e) + ch(e, f, g) + K[t] + W[t]) >>> 0;
      const T2 = (sigma0(a) + maj(a, b, c)) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + T1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (T1 + T2) >>> 0;
    }
    H[0] = (H[0] + a) >>> 0;
    H[1] = (H[1] + b) >>> 0;
    H[2] = (H[2] + c) >>> 0;
    H[3] = (H[3] + d) >>> 0;
    H[4] = (H[4] + e) >>> 0;
    H[5] = (H[5] + f) >>> 0;
    H[6] = (H[6] + g) >>> 0;
    H[7] = (H[7] + h) >>> 0;
  }

  const result = new Uint8Array(32);
  const resView = new DataView(result.buffer);
  for (let i = 0; i < 8; i++) {
    resView.setUint32(i * 4, H[i], false);
  }
  return result;
}

export async function decryptData(
  encryptedData: Uint8Array,
  password: string,
  salt: Uint8Array
): Promise<Uint8Array> {
  const length = encryptedData.length;
  const keyStream = new Uint8Array(length);
  const saltHex = Array.from(salt).map(b => b.toString(16).padStart(2, '0')).join('');
  const keyMaterialStr = password + saltHex;
  const keyMaterialBytes = new TextEncoder().encode(keyMaterialStr);

  let offset = 0;
  let counter = 0;

  while (offset < length) {
    const counterStr = String(counter);
    const counterBytes = new TextEncoder().encode(counterStr);
    const combined = new Uint8Array(keyMaterialBytes.length + counterBytes.length);
    combined.set(keyMaterialBytes);
    combined.set(counterBytes, keyMaterialBytes.length);

    const hashBuffer = await sha256(combined);
    const hashArray = new Uint8Array(hashBuffer);
    const chunkLen = Math.min(32, length - offset);
    keyStream.set(hashArray.subarray(0, chunkLen), offset);
    offset += chunkLen;
    counter++;
  }

  const decrypted = new Uint8Array(length);
  for (let i = 0; i < length; i++) {
    decrypted[i] = encryptedData[i] ^ keyStream[i];
  }
  return decrypted;
}